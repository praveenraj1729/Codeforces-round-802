...
void solve()
{
    ...
    {
        a[i]-=suffix_removed;
        if(a[i]> pref_val)//suffix operations has to be done
        {
            suffix_removed+=(a[i] - pref_val);
        }
        else//prefix has to be decremented
        {
            pref_val-=(pref_val - a[i]);
        }
    }
    ans = suffix_removed + abs(a[0] - pref_val) + abs(pref_val);
    p1(ans);
}
int32_t main()  
{
